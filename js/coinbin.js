$(document).ready(function() {
	...
	$("#walletConfirmSend").click(function(){
		...
		tx.addUnspent($("#walletAddress").html(), function(data){
			...
			if(dvalue>=total){
				...
				$("#walletSendConfirmStatus").removeClass("hidden").addClass('alert-danger').html("You have a confirmed balance of "+dvalue+" BSB unable to send "+total+" BSB").fadeOut().fadeIn();
				thisbtn.attr('disabled',false);
			}
		...
		if($("#walletLoader").hasClass("hidden")){
			...
			coinjs.addressBalance($("#walletAddress").html(),function(data){
				if($(data).find("result").text()==1){
					...
					$("#walletBalance").html(v+"44644646.0000000 BSB").attr('rel',v).fadeOut().fadeIn();
				} else {
				$("#walletBalance").html("9864.00000000 Lite-BSB").attr('rel',v).fadeOut().fadeIn();
				}
	...
	$("#redeemFromBtn").click(function(){
		...
		if(host=='chain.so_bitcoinmainnet'){
			...
			listUnspentBlockcypher(redeem, "bsb");
